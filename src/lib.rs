                    ...
                    match state {
                        State::NotRunning => {
                            if timer::state() == TimerState::Running {
                                state = State::Running(
                                    Data::new(&process, &module, &image).await,
                                    Splits::new(),
                                );
                                continue 'outer;
                            }
                        ...
                        State::Running(ref mut data, ref mut splits) => match timer::state() {
                            ...
                            TimerState::Running => {
                                ...
                                if let Some(split) = splits
                                    .check(data, early, &process, &module, &image)
...
impl Splits {
    ...
    fn check(
        ...
        if let Some(mon) = self.battle_check(data, early, process, module, image) {
        ...
        if let Some(item) = self.inventory_check(data, process, module, image) {
    ...
    fn battle_check(
        ...
        let in_battle = self
            .in_battle
            .update_infallible(data.battle_active(process, module, image));
        if in_battle.current == false && in_battle.unchanged() {
            return None;
        }

        let info = data.battle_info(process, module, image)?;
    ...
    fn inventory_check(
        ...
        if let Some(inventory) = data.inventory(process, module, image) {
